...
*/
class Move {
    constructor(piece1, pos1, piece2, pos2, special_move) {
...
class Engine {
    constructor(board, color) {
        this.king_moved = 0; // for checking if castling is valid at all
        this.lrook_moved = 0; // "lcastle" validation
        this.rrook_moved = 0; // "rcastle" validation
        this.pawn_dmoved = 0; // for checking if "en passant" is valid
    ...
    searchPieceMoves(pos) {
        ...
        switch (piece.toLowerCase()) {
            case "p":
                ...
                if (!color) {
                    if (x == 6 && this.board[x - 1][y] == " ")
                        p_unverified_coords.push([x - 2, y, 0]);
                    p_unverified_coords.push([x - 1, y, 0]);
                    // TODO: en passant if
                }
                else {
                    if (x == 1 && this.board[x + 1][y] == " ")
                        p_unverified_coords.push([x + 2, y, 0]);
                    p_unverified_coords.push([x + 1, y]);
                    // TODO: en passant if
                }
                p_unverified_coords.forEach(arr => {
                    if (arr[0] >= 0 && arr[0] < 8 && arr[1] >= 0 && arr[1] < 8 && (this.board[arr[0]][arr[1]] == " " /* en passant rule would go here i think */))
                        moves.push(new Move(piece, pos, this.board[arr[0]][arr[1]], [arr[0], arr[1]], ''));
                });
                break;
            case "n":
                let n_unverified_coords = [[x + 2, y + 1], [x + 2, y - 1], [x - 2, y + 1], [x - 2, y - 1], [x + 1, y + 2], [x + 1, y - 2], [x - 1, y + 2], [x - 1, y - 2]];
                n_unverified_coords.forEach(arr => {
                    if (arr[0] >= 0 && arr[0] < 8 && arr[1] >= 0 && arr[1] < 8 && this.getColor(this.board[arr[0]][arr[1]]) != color)
                        moves.push(new Move(piece, pos, this.board[arr[0]][arr[1]], arr, ''));
                });
                break;
            case "b":
                let b_arr = [x, y];
